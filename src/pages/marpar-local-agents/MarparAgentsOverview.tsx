...
import { MetricGroupCard, MetricGroupCardHeader } from '@sfe/react-metric-group-card'
import { Metric, MetricData, MetricTrend } from '@sfe/react-metric'
import Agents from './MarparAgentsAgents'
...
export default function MarparAgentsOverview() {
  ...
  const [externalFilter, setExternalFilter] = useState<{ definitionId: string; kind: string; selectedOptionId?: string | null; selectedOptionIds?: string[] } | null>(null);

  const agentsRef = React.useRef<HTMLDivElement>(null);

  const handleMetricClick = (filter: { definitionId: string; kind: string; selectedOptionId?: string | null; selectedOptionIds?: string[] }) => {
    setExternalFilter({ ...filter });
    // Scroll to the agents table
    setTimeout(() => agentsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
  };
  ...
  const metricsData = React.useMemo(() => {
    ...
    const isLocal = activeTab === 'devices';
    const localDimensions = isLocal ? {
    ...
    return { total, active, inactive, highRisk, mediumRisk, lowRisk, oversharing, exfiltration, unethical, tabLabel, isLocal, localDimensions };
  ...
  return (
    <>
      ...
      <main className="flex-1 flex flex-col p-[16px] pb-0 overflow-hidden bg-[#F0F0F0]">
        <div className="w-full flex-1 min-h-[500px] overflow-auto bg-white rounded-t-[12px] pt-[32px] px-[32px] pb-0 shadow-[0_0_2px_rgba(0,0,0,0.12),0_2px_8px_rgba(0,0,0,0.14)]">
          ...
          <TabList
            ...
            onTabSelect={(_event: SelectTabEvent, data: SelectTabData) => setActiveTab(data.value as 'cloud' | 'devices' | 'aiapps')}
            className="mb-6"
          >
            <Tab value="cloud" icon={
            ...
            }>{`Cloud Agents (${tabCounts.cloud})`}</Tab>
            <Tab value="devices" icon={
            ...
            }>{`Local Agents (${tabCounts.devices})`}</Tab>
            <Tab value="aiapps" icon={
          ...
          <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-5">
            {/* Card 1: Total agents */}
            <MetricGroupCard
              header={{ heading: metricsData.isLocal
                ? `${metricsData.total} ${metricsData.tabLabel} across`
                : `${metricsData.total} ${metricsData.tabLabel}` }}
              groupContent={{
                layout: metricsData.isLocal ? '3-up' : '2-up',
                breakpoints: 's',
                children: metricsData.isLocal && metricsData.localDimensions ? (
                  <>
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'platform', kind: 'single-select', selectedOptionId: null })}
                      metricTitle={{ label: 'Agent types', info: 'Number of unique agent types' }}
                      metricData={{ metric: { children: metricsData.localDimensions.uniqueAgents }, trend: { children: <MetricTrend value="2 new" direction="up" sentiment="negative" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'userName', kind: 'single-select', selectedOptionId: null })}
                      metricTitle={{ label: 'Users', info: 'Number of unique users' }}
                      metricData={{ metric: { children: metricsData.localDimensions.uniqueUsers }, trend: { children: <MetricTrend value="15%" direction="up" sentiment="negative" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'device', kind: 'single-select', selectedOptionId: null })}
                      metricTitle={{ label: 'Devices', info: 'Number of unique devices' }}
                      metricData={{ metric: { children: metricsData.localDimensions.uniqueDevices }, trend: { children: <MetricTrend value="8%" direction="up" sentiment="negative" /> } }}
                    />
                  </>
                ) : (
                  <>
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'status', kind: 'single-select', selectedOptionId: 'Active' })}
                      metricTitle={{ label: 'Active', info: 'Active agents in the last 30 days' }}
                      metricData={{ metric: { children: metricsData.active }, trend: { children: <MetricTrend value="3%" direction="up" sentiment="positive" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'status', kind: 'single-select', selectedOptionId: 'Inactive' })}
                      metricTitle={{ label: 'Inactive', info: 'Inactive agents' }}
                      metricData={{ metric: { children: metricsData.inactive }, trend: { children: <MetricTrend value="No change" direction="neutral" sentiment="neutral" /> } }}
                    />
                  </>
                ),
              }}
            ...
            {/* Card 2: High risk agents */}
            <MetricGroupCard
              header={{ heading: `${metricsData.highRisk} high risk ${metricsData.isLocal ? '' : (metricsData.tabLabel?.split(' ').pop() || 'agents')}` }}
              groupContent={{
                layout: '3-up',
                breakpoints: 's',
                children: (
                  <>
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'riskLevel', kind: 'single-select', selectedOptionId: 'High' })}
                      metricTitle={{ label: 'High risk' }}
                      metricData={{ metric: { children: metricsData.highRisk }, trend: { children: <MetricTrend value="25%" direction="up" sentiment="negative" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'riskLevel', kind: 'single-select', selectedOptionId: 'Medium' })}
                      metricTitle={{ label: 'Medium risk' }}
                      metricData={{ metric: { children: metricsData.mediumRisk }, trend: { children: <MetricTrend value="No change" direction="neutral" sentiment="neutral" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'riskLevel', kind: 'single-select', selectedOptionId: 'Low' })}
                      metricTitle={{ label: 'Low risk' }}
                      metricData={{ metric: { children: metricsData.lowRisk }, trend: { children: <MetricTrend value="10%" direction="up" sentiment="positive" /> } }}
                    />
                  </>
                ),
              }}
            ...
            {/* Card 3: Risky interactions */}
            <MetricGroupCard
              header={{ heading: `${metricsData.oversharing + metricsData.exfiltration + metricsData.unethical}${metricsData.isLocal ? '' : ` ${metricsData.tabLabel?.split(' ').pop() || 'agents'}`} with risky interactions` }}
              groupContent={{
                layout: '3-up',
                breakpoints: 's',
                children: (
                  <>
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'riskTypes', kind: 'multi-select', selectedOptionIds: ['Oversharing'] })}
                      metricTitle={{ label: 'Oversharing', info: 'Agents sharing data beyond scope' }}
                      metricData={{ metric: { children: metricsData.oversharing }, trend: { children: <MetricTrend value="120%" direction="up" sentiment="negative" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'riskTypes', kind: 'multi-select', selectedOptionIds: ['Exfiltration'] })}
                      metricTitle={{ label: 'Exfiltration', info: 'Agents with data exfiltration risk' }}
                      metricData={{ metric: { children: metricsData.exfiltration }, trend: { children: <MetricTrend value="No change" direction="neutral" sentiment="neutral" /> } }}
                    />
                    <Metric
                      style={{ cursor: 'pointer' }}
                      onClick={() => handleMetricClick({ definitionId: 'riskTypes', kind: 'multi-select', selectedOptionIds: ['Unethical'] })}
                      metricTitle={{ label: 'Unethical', info: 'Agents with unethical behavior risk' }}
                      metricData={{ metric: { children: metricsData.unethical }, trend: { children: <MetricTrend value="No change" direction="neutral" sentiment="neutral" /> } }}
                    />
                  </>
                ),
              }}
          ...
          <div ref={agentsRef} className="mt-2 text-[#616161] overflow-hidden">
            <Agents activeTab={activeTab} groupBy={groupBy} externalFilter={externalFilter} />
          </div>
        </div>
      </main>
    </>
