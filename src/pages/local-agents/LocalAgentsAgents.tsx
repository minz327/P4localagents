...
function AgentRow_({ row, navigate, tab, columns }: { row: AgentRow; navigate: (path: string) => void; tab: 'cloud' | 'devices' | 'aiapps'; columns: { label: string; key: string }[] }) {
  ...
  const renderCell = (key: string) => {
    switch (key) {
      ...
      case 'platform': return <td key={key} className="px-6 py-2 text-[#242424] text-[13px]">{row.hosting === 'local' ? <div className="flex items-center gap-2"><AppIcon type={row.type} initials={row.initials} icon={row.icon} platform={row.platform} /><span>{row.platform}</span></div> : (row.platform || '\u2014')}</td>;
  ...
  return (
    <div className="bg-white rounded-none border-t border-gray-200 mt-4 font-sans">
      ...
      <div className="flex items-center px-6 py-3 gap-2 border-b border-gray-100 text-[13px] bg-white flex-wrap">
        ...
        <FilterPill label={activeTab === 'devices' ? 'Agent type' : 'Platform'} value={filters.platform} options={filterOptions.platform} onChange={v => setFilters(f => ({ ...f, platform: v }))} />
