...
import EastmanAgents, { GovernanceFilter } from './EastmanAgents'
import { demoAgents } from '../../lib/eastmanData'
...
function GovernanceMetrics({ activeFilter, onCardClick }: { activeFilter: GovernanceFilter; onCardClick: (filter: GovernanceFilter) => void }) {
  const needsReview = demoAgents.filter(a => a.governanceState !== 'Reviewed').length
  const noDLP = demoAgents.filter(a => !a.hasDLP).length
  const changedSince = demoAgents.filter(a => a.governanceState === 'Changed since review').length
  const neverReviewed = demoAgents.filter(a => a.governanceState === 'Never reviewed').length

  const cards: { label: string; filterKey: GovernanceFilter; value: number; total: number; color: string; bg: string; icon: React.ReactNode }[] = [
    { label: 'Agents needing review', filterKey: 'needsReview', value: needsReview, total: demoAgents.length, color: 'text-[#A4262C]', bg: 'bg-[#FDE7E9]', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="#A4262C"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg> },
    { label: 'Without policy coverage', filterKey: 'noPolicy', value: noDLP, total: demoAgents.length, color: 'text-[#D83B01]', bg: 'bg-[#FFF4CE]', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="#D83B01"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm1 15h-2v-2h2v2zm0-4h-2V7h2v5z"/></svg> },
    { label: 'Changed since review', filterKey: 'changedSinceReview', value: changedSince, total: demoAgents.length, color: 'text-[#835C00]', bg: 'bg-[#FFF4CE]', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="#835C00"><path d="M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z"/></svg> },
    { label: 'Never reviewed', filterKey: 'neverReviewed', value: neverReviewed, total: demoAgents.length, color: 'text-[#A4262C]', bg: 'bg-[#FDE7E9]', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="#A4262C"><circle cx="12" cy="12" r="10"/><path d="M13 7h-2v6h2V7zm0 8h-2v2h2v-2z" fill="white"/></svg> },
  ]
        ...
        return (
          <div
            ...
            className={`bg-white border rounded-lg p-4 shadow-sm cursor-pointer transition-all hover:shadow-md ${
              isActive ? 'border-[#0078D4] ring-2 ring-[#0078D4]/20' : 'border-gray-200'
            }`}
          >
            <div className="flex items-center gap-2">
              ...
              <div className="text-[12px] text-[#616161] font-medium">{card.label}</div>
            </div>
            <div className="mt-3 flex items-baseline gap-1">
              <span className={`text-[28px] font-bold ${card.color}`}>{card.value}</span>
              <span className="text-[14px] text-[#616161]">/ {card.total}</span>
            </div>
            {/* Progress bar */}
            <div className="mt-2 h-1.5 bg-[#F3F2F1] rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${card.value > 0 ? 'bg-[#A4262C]' : 'bg-[#107C10]'}`}
...
export default function EastmanOverview() {
  const [governanceFilter, setGovernanceFilter] = useState<GovernanceFilter>(null)
  const tableRef = useRef<HTMLDivElement>(null)

  const metricsData = useMemo<MetricsData>(() => ({
    totalApps: demoAgents.length,
    active: demoAgents.filter(a => a.status === 'Active').length,
    inactive: demoAgents.filter(a => a.status === 'Inactive').length,
    highRisk: demoAgents.filter(a => a.riskLevel === 'High').length,
    mediumRisk: demoAgents.filter(a => a.riskLevel === 'Medium').length,
    lowRisk: demoAgents.filter(a => a.riskLevel === 'Low').length,
    sensitive: {
      oversharing: demoAgents.filter(a => a.riskTypes.includes('Oversharing')).length,
      exfiltration: demoAgents.filter(a => a.riskTypes.includes('Exfiltration')).length,
      unethical: demoAgents.filter(a => a.riskTypes.includes('Unethical')).length,
    },
  }), [])

  const handleCardClick = (filter: GovernanceFilter) => {
    setGovernanceFilter(filter)
      ...
      <main className="flex-1 px-6 py-6 overflow-auto">
        <div className="w-full">
          ...
          <div className="mb-4 mt-10">
            <div className="text-[18px] font-semibold text-[#242424]">Governance posture</div>
            <div className="text-[14px] text-[#242424] mt-1">Agent governance status and policy coverage across your organization.</div>
          </div>

          <GovernanceMetrics activeFilter={governanceFilter} onCardClick={handleCardClick} />
          
          <Toolbar />

          <div ref={tableRef} className="mt-2 text-[#616161]">
            <EastmanAgents activeFilter={governanceFilter} onClearFilter={() => setGovernanceFilter(null)} />
          </div>
        </div>
      </main>
    </>
