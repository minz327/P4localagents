...
/*
 * Enhanced Metrics component — clean cards without scattered anomaly badges.
 * Anomaly signals are now consolidated in the CopilotPromptBar instead.
 */
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-white p-6 rounded-md shadow-sm min-h-[170px] flex flex-col border border-gray-200">
      {children}
    </div>
  )
}

export default function ClevelandMetrics({ onSpikeClick }: { onSpikeClick?: () => void }) {
  const data = {
    totalApps: 254,
    active: 254,
    inactive: 0,
  ...
  return (
    <section>
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        {/* Card 3: Sensitive interactions — with SPIKE badge */}
        <div onClick={onSpikeClick} className={`bg-white p-6 rounded-md shadow-sm min-h-[170px] flex flex-col border border-gray-200 ${onSpikeClick ? 'cursor-pointer hover:border-[#C50F1F] hover:shadow-md transition-all' : ''}`}>
          <div className="text-[16px] font-semibold text-[#242424] pb-4 border-b border-[#E0E0E0] mb-5 flex items-center gap-2">
            {data.sensitive.oversharing + data.sensitive.exfiltration + data.sensitive.unethical} agents with sensitive interactions
            <span className="px-2 py-0.5 bg-[#FDE7E9] text-[#C50F1F] text-[10px] font-bold rounded-full flex items-center gap-1">
              ⚡ SPIKE
            </span>
          </div>
